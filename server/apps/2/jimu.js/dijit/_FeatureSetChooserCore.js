...
define([
  ...
  function (on, sniff, Evented, Deferred, lang, array, declare, jimuUtils, symbolUtils, SelectionManager,
    LayerInfos, Graphic, EsriQuery, QueryTask, FeatureLayer, symbolJsonUtils, geometryEngine, topic) {
    ...
    return declare([Evented], {
      ...
      _onDrawEnd: function (g, geotype, commontype, shiftKey, ctrlKey, metaKey) {
        ...
        this._getFeaturesByGeometry(g.geometry).then(lang.hitch(this, function (features) {
          ...
          console.log("Selection method was: " + selectionMethod)

          switch (selectionMethod) {
            case FeatureLayer.SELECTION_SUBTRACT:
              for (let i = 0; i < selectedFeatures.length; i++)
                window.IDArr.filter(n => !features.some(f => f.attributes.OBJECTID === n));
              break;
            case FeatureLayer.SELECTION_ADD:
              for (let i = 0; i < features.length; i++)
                if (IDArr.includes(features[i].attributes.OBJECTID))
                  window.IDArr.push(features[i].attributes.OBJECTID);
              break;
            case FeatureLayer.SELECTION_NEW:
              if (window.lastExtent != g._extent)
                window.IDArr = [];
              for (let i = 0; i < features.length; i++)
                window.IDArr.push(features[i].attributes.OBJECTID);
              break;
          }
          topic.publish('publishData', "selectOR", "selectOR", window.IDArr, false);
          console.log("IDArr object equal to: " + window.IDArr);
      ...
      _onDrawBoxUserClear: function () {
        ...
        topic.publish('publishData', "selectCLR", "selectCLR", 0, false);
        this.clear();
      },
