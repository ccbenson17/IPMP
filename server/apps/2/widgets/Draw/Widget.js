...
/**
 * @fileoverview
 * Provides an interface for the user to add elements to the main map: e.g. draw a line or shape overlay on the map. ({@link module:Draw|More info})
 */
define([
	'dojo/_base/declare',
	...
	return declare([BaseWidget, _WidgetsInTemplateMixin], {
		...
		saveDrawToolTips: function () {
			...
			if (this.map.snappingManager) {
				this.suffixStr = '<br/>' + '(' + this.jimuNls.snapping.pressStr + '<b>' + this.jimuNls.snapping.ctrlStr + '</b> ' + this.jimuNls.snapping.snapStr + ')';
			}
		},
		...
		_addLayerFromLayerInfos: function (layer) {
			...
			LayerInfos.getInstance(this.map, this.map.itemInfo).then(
				lang.hitch(this, function (layerInfos) {
					if (!this.domNode) {
						return;
					}
					loading.destroy();
		...
		_onDrawEnd: function (graphic, geotype, commontype) {
			...
			this.drawBox.clear();
			if (!graphic.symbol) {
				//not draw and save graphic that has null symbol.
				return;
			}
		...
		_initUnitSelect: function () {
			...
			array.forEach(
				this.locationUnits,
				lang.hitch(this, function (unitInfo) {
					var option = {
						value: unitInfo.unit,
		...
		getCoordValues: function (fromInput, toType, numDigits) {
			...
			this._gs.toGeoCoordinateString(params).then(
				function (itm) {
		...
		_getSymbolFont: function (textFontSizeDijit) {
			var a = Font.STYLE_ITALIC,
				b = Font.VARIANT_NORMAL,
				c = Font.WEIGHT_NORMAL;
		...
		_addLocationMeasure: function (geometry, graphic) {
			...
			var abbr = unitInfo.abbrTag,
				format = unitInfo.format;
			this.getDDPoint(geometry).then(
				lang.hitch(this, function (mapPoint) {
					this.getCoordValues(mapPoint, abbr, 4).then(
						lang.hitch(this, function (r) {
		...
		_addLineMeasure: function (geometry, graphic) {
			this._getLengthAndArea(geometry, false).then(
		...
		_addPolygonMeasure: function (geometry, graphic, ifArea, ifLength) {
			this._getLengthAndArea(geometry, true).then(
				lang.hitch(this, function (result) {
					if (!this.domNode) {
						return;
					}
					...
					if (ifArea) {
						...
						var labelGraphic1 = this._getLabelGraphic(areaLabel, unitAbbr1, result.area, center, symbolFont1, fontColor1, offsetY1);
					...
					if (ifLength) {
						...
						var labelGraphic2 = this._getLabelGraphic(distanceLabel, unitAbbr2, result.length, center, symbolFont2, fontColor2, offsetY2);
		...
		_getLengthAndAreaByGS: function (geometry, isPolygon, esriAreaUnit, esriLengthUnit) {
			...
			if (isPolygon) {
				...
				this._gs.simplify([geometry]).then(
					lang.hitch(this, function (simplifiedGeometries) {
						if (!this.domNode) {
							return;
						}
						areasAndLengthParams.polygons = simplifiedGeometries;
						this._gs.areasAndLengths(areasAndLengthParams).then(
							lang.hitch(this, function (result) {
								if (!this.domNode) {
									return;
								}
				...
				this._gs.lengths(lengthParams).then(
					lang.hitch(this, function (result) {
						if (!this.domNode) {
							return;
						}
