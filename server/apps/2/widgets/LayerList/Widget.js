...
/**
 * @fileoverview
 * Displays all layers in the map, populated by the active basemap & results from queries. ({@link module:LayerList|More info})
 */
define([
	'jimu/BaseWidget',
	'dojo/_base/declare',
...
], function (BaseWidget, declare, lang, array, html, dom, on, query, registry, LayerListView, LayerFilter, NlsStrings, LayerInfos) {
	/**
	 * @module LayerList
	 */
	var clazz = declare([BaseWidget], {
		...
		startup: function () {
			...
			if (this.map.itemId) {
				...
				LayerInfos.getInstance(this.map, itemInfo).then(
					lang.hitch(this, function (operLayerInfos) {
						this.operLayerInfos = operLayerInfos;
						this.showLayers();
						this.bindEvents();
		...
		_onLayerInfosScaleRangeChanged: function (changedLayerInfos) {
			array.forEach(
				...
				function (layerInfo) {
					var layerInfoArray = [];
					layerInfo.traversal(
						lang.hitch(this, function (subLayerInfo) {
