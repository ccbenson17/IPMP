...
define(['dojo/_base/declare',
  ...
  function (declare, lang, html, on, keys, domStyle, domClass, domConstruct, domGeometry, mouse,
    baseFx, coreFx, Move, Deferred, ResizeHandle, utils, BaseWidgetPanel, _TemplatedMixin, template, WidgetManager, array) {
    /* global jimuConfig*/
    ...
    return declare([BaseWidgetPanel, _TemplatedMixin], {
      ...
      _onCloseBtnClicked: function (evt) {
        var headWidget = WidgetManager.getInstance().getWidgetByLabel(this._getWidgetConfig('AnchorBarController', true).label);
        var pickCols = WidgetManager.getInstance().getWidgetByLabel("PickColumns_2");
        if (pickCols != null && this.config.name == 'myInfographic') {
          pickCols.selectGraph.set('value', "Choose one:");
          pickCols.selectAVG.set('value', "Choose one:");
          pickCols.selectMIN.set('value', "Choose one:");
        }

        evt.stopPropagation();
        this.panelManager.closePanel(this, 'wipe');
        headWidget.resize();
        headWidget._removeFromOpenedIds([this.id]);
      },
      ...
      _normalize: function () {
        var box, width, def = new Deferred(),
          animArray = [];
